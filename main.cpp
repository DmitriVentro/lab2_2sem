...
int main()
{
    ...
    cout << "Программа будет выводить все процессы, которые будут происходить, когда мы будем запускать какие-либо функции, вместе с типом функции также будет выводиться "
    <<"адрес ячейки памяти для полноты отчёта:)"<<endl;
